@Sanity
Feature: Checkout with variant product Along with Buy X Get Y Base Percentage OFF coupon

  Scenario: Buy X And Get X Coupon Applied to Cart with Variant Item
    ...
    * def cart = call read('classpath:karate/core/cart/addToCart.feature@AddingSingleItemToCart') {ItemId: '#(ItemId1)', ItemAttributes:'#(ItemId1Attributes)', amount: '#(priceForItem1)'}
    * def cartId = cart.response.id
    * def lineItemId = cart.response.lineItems[0].id
    * def position = cart.response.lineItems[0].position


    * def cartQuantity = call read('classpath:karate/core/cart/updateQuantity.feature@UpdateQtyForSingleLineItem') {lineItemId: '#(lineItemId)', position: '#(position)', productId: '#(ItemId1)', cartId: '#(cartId)', Quantity: '3' }
    * def cartId = cart.response.id

    * def applyCoupon = call read('classpath:karate/core/cart/applyCoupon.feature@SingleLineItem') {cartId: '#(cartId)', couponCode : '#(Buy_X_Get_Y_Coupon)'}
