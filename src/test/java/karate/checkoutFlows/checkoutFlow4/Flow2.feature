@Sanity
Feature: Checkout without Variant product, Buy X Get Y

  Scenario: Buy X And Get X Coupon Applied to Cart With Non Variant Item
    ...
    * def cart = call read('classpath:karate/core/cart/addToCart.feature@AddingSingleItemToCart') {ItemId: '#(ItemId)', ItemAttributes:'#(ItemAttributes)', amount: '#(priceForItem1)'}
    * def cartId = cart.response.id
    * def lineItemId = cart.response.lineItems[0].id
    * def position = cart.response.lineItems[0].position


    * def cartQuantity = call read('classpath:karate/core/cart/updateQuantity.feature@UpdateQtyForSingleLineItem') {lineItemId: '#(lineItemId)', position: '#(position)' ,productId: '#(ItemId)', cartId: '#(cartId)', Quantity: '3' }
    * def cartId = cart.response.id
