...
Feature: Checkout with variant product Along with Buy X Get Y  AMT OFF Promotion
  ...
  Scenario: BUY  X And Get Y Promotion Applied to Cart with Variant Item
    ...
    * def cart = call read('classpath:karate/core/cart/addToCart.feature@PromotionalCartWithSingleLineItemWithMultiPleQua') {ItemId: '#(ItemId1)', ItemAttributes:'#(ItemId1Attributes)', quantity:'3'}
