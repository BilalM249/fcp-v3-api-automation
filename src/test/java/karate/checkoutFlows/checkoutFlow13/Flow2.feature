@Sanity
Feature: Checkout without Variant Promotion Quantity product

  Scenario: Quantity promotions and no coupons With Non Variant Item
    * def product  = call read('classpath:karate/core/pim/pim.feature@ForNonVariantItem') {ItemId : '#(FCP_QUA_Promotional_Non_Variant_ItemId)'}
    ...
    * def cart = call read('classpath:karate/core/cart/addToCart.feature@PromotionalCartWithSingleLineItemWithMultiPleQua') {ItemId: '#(ItemId)', ItemAttributes:'#(ItemAttributes)',  quantity:'2'}
