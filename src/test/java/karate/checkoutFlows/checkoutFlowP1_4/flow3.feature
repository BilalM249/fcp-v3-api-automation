...
Feature: Shipping Promotion applicable on SKU
  ...
  Scenario: Shipping Promotion applicable on SKU with free shipping
    ...
    * def getCart = call read('classpath:karate/core/cart/getCart.feature@GetCartWithSingleItemAndFreeShipping') {cartID: '#(cartId)' }

    * def cartAmount = getCart.response.totalAmount
    * def shippingCost = getCart.response.lineItems[0].shippingDetails.shippingCost
